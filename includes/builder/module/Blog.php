	...
	static function get_blog_posts( $args = array(), $conditional_tags = array(), $current_page = array() ) {
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				// Print output
				?>
					<article id="" <?php post_class( 'et_pb_post child clearfix' . $no_thumb_class . $overlay_class ); ?>>
						<?php
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( have_posts() ) {
			...
			while ( have_posts() ) {
				...
				$item_class = sprintf( ' et_pb_blog_item_%1$s', $items_count );
				$items_count++;
				?>

			<article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post child clearfix' . $no_thumb_class . $overlay_class . $item_class ); ?>>
